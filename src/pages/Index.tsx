          ...
          <div className="mt-12 flex flex-wrap justify-center gap-12 md:gap-16">
            {[
              ...
              <Section key={cat.name}>
                <Link to={`/shop?category=${cat.filter}`} className="group flex flex-col items-center gap-5">
                  <div className="relative h-52 w-52 md:h-60 md:w-60">
                    {/* Outer decorative ring */}
                    <div className="absolute inset-0 rounded-full border-2 border-primary/40 transition-all duration-500 group-hover:border-primary group-hover:scale-105" />
                    {/* Gap ring */}
                    <div className="absolute inset-[6px] rounded-full border border-accent/20" />
                    {/* Image circle */}
                    <div className="absolute inset-[10px] overflow-hidden rounded-full shadow-lg">
                      <img
                        src={cat.image}
                        alt={cat.name}
                  ...
                  <div className="text-center">
                    <h3 className="font-serif text-xl font-bold text-foreground tracking-wide">{cat.name}</h3>
                    <span className="mt-1 inline-block text-sm font-medium text-accent transition-all group-hover:tracking-wider">Shop {cat.name} →</span>
                  </div>
                </Link>
              </Section>
            ))}
